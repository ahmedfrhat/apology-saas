...
import CinematicDocumentaryIntro from "@/components/CinematicDocumentaryIntro";
...
export default function SaaSOnboardingPage() {
  ...
  // Cinematic Short Documentary Reels Intro state
  const [showDocumentaryIntro, setShowDocumentaryIntro] = useState(false);
  ...
  // Hydrate global stats, documentary check, and content
  useEffect(() => {
    setIsMounted(true);
    ...
    // Check if initial documentary was already watched or skipped
    if (localStorage.getItem("safi_documentary_watched") !== "true") {
      setShowDocumentaryIntro(true);
    }
  ...
  const handleReplayDocumentaryIntro = () => {
    localStorage.removeItem("safi_documentary_watched");
    setShowDocumentaryIntro(true);
  };
  ...
  return (
    <div className="flex min-h-screen flex-col bg-[#FCFBF7] dark:bg-gray-950 font-sans antialiased text-gray-900 dark:text-gray-100 relative transition-colors">
      ...
      {/* Dynamic Master Documentary Reel Overlay Hook */}
      <AnimatePresence>
        {showDocumentaryIntro && (
          <CinematicDocumentaryIntro 
            onFinishIntro={() => setShowDocumentaryIntro(false)} 
          />
        )}
      </AnimatePresence>

      {/* Main Beautiful Gate Transition Triggered after Documentary finishes */}
      {!showDocumentaryIntro && (
        <motion.div
          key="master-portal"
        ...
        >
          ...
          <AnimatePresence>
            {adBlockDetected && !hideAdBanner && (
              <motion.div 
                ...
                exit={{ opacity: 0, y: -20 }}
                className="bg-amber-50 dark:bg-amber-950/80 border-b border-amber-200 dark:border-amber-900 p-3 flex items-center justify-center text-center shadow-md relative z-50 transition-colors"
              >
                <div className="flex items-center gap-3 max-w-2xl mx-auto w-full justify-between">
                  <span className="text-amber-800 dark:text-amber-300 text-xs sm:text-sm font-black">
                    {t("adblockText")}
                  </span>
                  <button 
          ...
          <header className="w-full max-w-7xl mx-auto px-4 sm:px-6 py-5 flex flex-wrap items-center justify-between gap-4 z-40 relative border-b border-gray-200 dark:border-gray-800 shadow-sm bg-white/80 dark:bg-gray-950/80 backdrop-blur-xl transition-colors">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={handleReplayDocumentaryIntro}
                className="w-8 h-8 rounded-full bg-gray-100 hover:bg-gray-200 dark:bg-gray-900 dark:hover:bg-gray-800 flex items-center justify-center text-gray-500 dark:text-gray-400 active:scale-95 transition-transform cursor-pointer"
                title={locale === "en" ? "Replay Cinematic Reel Intro" : "إعادة مشاهدة الفيديو التوثيقي 🎬"}
              >
                <RotateCcw size={14} className="animate-spin hover:text-amber-800" />
              </button>
            </div>
            ...
            <nav className="flex items-center gap-1 sm:gap-1.5 bg-gray-100 dark:bg-gray-900 p-1.5 rounded-full border border-gray-200 dark:border-gray-800 shadow-inner overflow-x-auto max-w-full font-mono transition-colors">
              <button
                ...
                className={`${TAB_BTN} ${
                  activeTab === "wall"
                    ? "bg-white dark:bg-gray-800 text-amber-800 dark:text-amber-400 shadow-md"
              ...
              <button
                ...
                className={`${TAB_BTN} ${
                  activeTab === "create"
                    ? "bg-gradient-to-r from-amber-800 to-[#B45309] text-white shadow-xl shadow-amber-800/25"
                    : "text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white"
                }`}
              >
            ...
            <div className="flex items-center gap-2 sm:gap-3 ml-auto sm:ml-0 font-mono">
              <button
                ...
                className="px-4.5 py-2.5 rounded-full text-xs sm:text-sm font-black transition-all bg-amber-800/10 dark:bg-amber-900/30 text-amber-800 dark:text-amber-300 hover:scale-105 active:scale-95 cursor-pointer flex items-center gap-1.5 shadow-sm border border-amber-800/20"
              >
          ...
          <main className="flex-1 w-full max-w-7xl mx-auto px-4 sm:px-6 py-10 sm:py-16 relative z-10">
            <AnimatePresence mode="wait">
              ...
              {activeTab === "wall" && (
                <motion.section 
                  ...
                  className={`${CARD} p-8 sm:p-12 flex flex-col h-[700px] max-w-4xl mx-auto relative overflow-hidden`}
                >
                  ...
                  <form onSubmit={handleApologySubmit} className="flex flex-col sm:flex-row gap-3 relative z-10 font-sans">
                    <div className="flex-1 relative">
                      <input 
                        ...
                        className="w-full bg-white dark:bg-gray-800 p-4.5 rounded-2xl border border-gray-300 dark:border-gray-700 text-xs sm:text-sm font-bold outline-none focus:border-indigo-600 dark:focus:border-indigo-400 dark:text-white shadow-inner transition-colors"
                      />
                    ...
                    <button 
                      ...
                      className="bg-indigo-600 hover:bg-indigo-700 active:scale-95 text-white text-xs sm:text-sm font-black px-10 py-4.5 rounded-2xl transition-all cursor-pointer disabled:opacity-50 flex items-center justify-center gap-2 shadow-lg shadow-indigo-600/25 font-mono"
                    >
                      {apologyLoading ? (
                        <>
              ...
              {activeTab === "stories" && (
                ...
                >
                  ...
                  {/* Relatable WhatsApp Story Hook demo prompt banner */}
                  ...
                  <div className={`${CARD} p-8 sm:p-12 relative overflow-hidden shadow-2xl font-sans`}>
                    ...
                    {success ? (
                      ...
                      <form onSubmit={handleSubmit} className="space-y-5 font-sans">
                        ...
                        <div className="border border-gray-300 dark:border-gray-700 rounded-2xl bg-white dark:bg-gray-800 overflow-hidden shadow-sm font-mono">
                          <button 
                            type="button" 
          ...
          <div className="fixed inset-0 z-[110] flex items-center justify-center p-5 select-none font-sans">
            ...
            >
              ...
              <form onSubmit={handleLoginSubmit} className="space-y-4">
                {loginError && (
                  <div className="p-4 bg-red-50 dark:bg-red-950 text-red-700 dark:text-red-300 rounded-2xl text-xs font-black border border-red-200 dark:border-red-800 text-center shadow-sm">
                    {loginError}
                  </div>
                )}
                ...
                <div className="font-sans">
                  ...
                  <input
                    ...
                    placeholder="••••••••"
                    className="w-full rounded-2xl border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 p-4 text-xs sm:text-sm font-black outline-none focus:border-amber-800 shadow-inner font-sans"
                  />
                </div>
